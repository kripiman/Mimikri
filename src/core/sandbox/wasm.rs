...
impl WasmRuntime {
    ...
    pub fn execute_plugin(&self, wasm_bytes: &[u8], input_json: &str) -> Result<String> {
        let module =
            Module::new(&self.engine, wasm_bytes).context("Failed to create WASM module")?;
        ...
        memory
            ...
            .map_err(|e| anyhow::anyhow!("WASM Memory Write Error: {:?}", e))?;
        ...
        scan.call(
            &mut store,
            &[Value::I32(ptr as i32), Value::I32(input_len as i32)],
            &mut output_results,
        )?;
        ...
        memory
            .read(&store, out_ptr, &mut len_buf)
            .map_err(|e| anyhow::anyhow!("WASM Memory Read Error (len): {:?}", e))?;
        let out_len = u32::from_le_bytes(len_buf) as usize;

        let mut out_buf = vec![0u8; out_len];
        memory
            .read(&store, out_ptr + 4, &mut out_buf)
            .map_err(|e| anyhow::anyhow!("WASM Memory Read Error (data): {:?}", e))?;
