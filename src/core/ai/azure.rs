...
impl LlmClient for AzureOpenAIClient {
    async fn analyze(
        ...
        let text = res["choices"][0]["message"]["content"]
            .as_str()
            .context("Azure response format error")?;
    ...
    async fn decide_action(
        ...
        let ctx = config
            ...
            .unwrap_or_default();
        let prompt_raw = format!(
            "Target: {}{}, Finding: {}, Context: {:?}",
            config.target.host, ctx, config.finding.id, config.adaptive_context
        );
        let prompt = crate::core::ai::caveman::CavemanOptimizer::optimize_prompt(
            &prompt_raw,
            config.caveman,
        ...
        let text = res["choices"][0]["message"]["content"]
            .as_str()
            .context("Azure decision format error")?;
