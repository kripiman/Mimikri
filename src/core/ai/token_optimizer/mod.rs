...
pub(crate) static WENYAN_MAP: Lazy<Vec<(Regex, &'static str)>> = Lazy::new(|| {
    vec![
...
pub(crate) static VERBOSITY_RULES: Lazy<Vec<(Regex, &'static str)>> = Lazy::new(|| {
    vec![
        ...
        (
            Regex::new(r"(?i)in spite of the fact that").unwrap(),
            "although",
        ),
