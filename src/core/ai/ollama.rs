...
impl LlmClient for OllamaClient {
    async fn analyze(
        ...
        let mut analysis: AIAnalysis =
            serde_json::from_value(self.base.parse_extraction(response_text)?)?;
        ...
        analysis.usage.total_tokens =
            analysis.usage.prompt_tokens + analysis.usage.completion_tokens;
    ...
    async fn decide_action(
        ...
        let compressed_finding =
            ContextCompressor::compress_finding(config.finding, config.route_level);
        ...
        let text = res["response"]
            .as_str()
            .context("Ollama decision missing text")?;
