...
impl<M: ExecutorMode> AutonomousAgent<M> {
    ...
    pub async fn run_autopilot(
        ...
        info!(
            "🤖 SENTINEL: Iniciando ciclo autónomo adaptativo para {}",
            initial_target.host
        );
        ...
        tokio::spawn(async move {
            let _ = pipeline.run_discovery(&target, tx_discovery).await;
        });
        ...
        while let Some(finding) = rx.recv().await {
            ...
            if let Some(ref log) = self.activity_log {
                let _ = log
                    .log(
                        crate::utils::activity_log::EventKind::AgentStep,
                        crate::utils::activity_log::Actor::Sentinel,
                        &format!("Processing finding: {}", finding.core.title),
                        Some(&initial_target.host),
                        serde_json::json!({"finding_id": finding.core.id}),
                    )
                    .await;
            }

            crate::core::correlation::ingestor::Ingestor::ingest_finding(
            ...
            let analysis = self
                .router
                .analyze(&finding, &initial_target, attack_context.as_deref())
                .await?;
            let mut final_finding = finding.with_ai_analysis(analysis.clone());
            if let Some(tags) = analysis.mitre_attack {
                final_finding = final_finding.with_mitre_attack(tags);
            }

            if analysis.risk_score >= 8
                || final_finding.core.severity == crate::models::Severity::High
                || final_finding.core.severity == crate::models::Severity::Critical
            {
            ...
            if !paths.is_empty() {
                Arc::make_mut(&mut sink_target.extra_data)["attack_paths"] =
            ...
            if let Ok(Some((action, tactical))) = self
                .router
                .decide_action(
                    &final_finding,
                    &initial_target,
                ...
                .await
            {
                if let Some(ref log) = self.activity_log {
                    let _ = log
                        .log(
                            crate::utils::activity_log::EventKind::ToolCall,
                            crate::utils::activity_log::Actor::Sentinel,
                            &format!("AI decided action: {}", action),
                            Some(&initial_target.host),
                            tactical.clone(),
                        )
                        .await;
            ...
            if adaptive_context.previous_actions.len() > 50 {
                break;
            }
        }
    ...
    async fn request_operator_approval(&self, action: &str) -> bool {
        ...
        matches!(
            self.approval_gate
                .request_approval(action, 85, &self.operator, "Autonomous Adaptive Loop")
                .await,
            Ok(None)
        )
    }
