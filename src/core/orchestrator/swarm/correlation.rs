...
pub fn process_correlation(
    ...
    paths
        .into_iter()
        .filter(|path| {
            !seen_ids.contains(&format!(
                "{}-{}",
                crate::models::constants::FINDING_ATTACK_PATH,
                path.pattern_signature()
            ))
        })
        .map(|path| {
            let signature = path.pattern_signature();
            let next_hop_host = path.nodes.iter().skip(1).find_map(|node_id| {
                let node = graph.nodes.get(node_id)?;
                let props = node.evidence.primary.as_ref()?.data.get("properties")?;
                let is_computer = node
                    .evidence
                    .primary
                    .as_ref()?
                    .data
