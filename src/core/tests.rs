use crate::core::ai::{RouteLevel, TieredAIRouter};
...
impl DataSink for MockSink {
    async fn write(&mut self, _: &TargetHost) -> Result<()> {
        Ok(())
    }
...
fn test_ai_waf_escalation() {
    ...
    let target_waf = TargetHost {
        ...
        findings: Arc::new(vec![Finding::new(
...
fn test_roe_testing_window() {
    use crate::core::policy::{PolicyProvider, RoE, StaticPolicy};
...
async fn test_objective_persistence() -> anyhow::Result<()> {
    ...
    use crate::models::{Objective, ObjectivePhase, ObjectiveStatus};
    ...
    let obj = Objective::new(
        "OBJ-001",
        "Initial Access",
        "Gain a foothold in the perimeter.",
...
fn test_dense_finding_encoding() {
    ...
    assert_eq!(
        compressed.get("id").unwrap().as_str().unwrap(),
        "FIND-DENSE-01"
    );
    assert_eq!(compressed.get("s").unwrap().as_str().unwrap(), "H");
    assert_eq!(
        compressed.get("cat").unwrap().as_str().unwrap(),
        "vulnerability"
    );
