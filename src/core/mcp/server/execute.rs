...
use crate::models::{Severity, TargetHost, TargetStatus, TargetType};
use crate::utils::tone::{to_ascii_safe, tone_encode, tonl_encode};
...
async fn log_tool_call(state: &Arc<McpServer>, tool: &str, input: &str) -> Option<String> {
    ...
    if loop_count >= 2 {
        Some(format!(
            "⚠️ [LOOP-DETECT] '{}' llamado {}x con el mismo input. Posible alucinación en cadena.",
            tool,
            loop_count + 1
        ))
    } else {
...
pub async fn handle_execute_plugin(
    state: &Arc<McpServer>,
    ...
    info!(
        "🛡️ [MCP-OPSEC] Interpolando '{}' -> Real: '{}'",
        target_masked, target_real
    );
    ...
    if let Some(cached) = state.plugin_cache.get(&secure_key) {
        ...
        info!(
            "🚀 [MCP-CACHE] Hit SEGURO en RAM para: {} (SHA256)",
            plugin_name
        );
        return json!(CallToolResult {
            content: vec![McpContent::Text {
                text: format!("[CACHED: RAM] {}", cached)
            }],
            is_error: false
        });
    }
    ...
    if let Some(cached) = state.plugin_cache.get(&legacy_key) {
        state.cache_hits.fetch_add(1, Ordering::Relaxed);
        warn!(
            "⚠️ [MCP-CACHE] Hit LEGACY en RAM para: {} (Migrando...)",
            plugin_name
        );
        state
            .plugin_cache
            .insert(secure_key.clone(), cached.clone())
            .await;
        return json!(CallToolResult {
            content: vec![McpContent::Text {
                text: format!("[CACHED: LEGACY_RAM] {}", cached)
            }],
            is_error: false
        });
    }
    ...
    if let Some(ref db) = state.db {
        if let Ok(Some(cached)) = db.load_plugin_cache(&secure_key).await {
            ...
            return json!(CallToolResult {
                content: vec![McpContent::Text {
                    text: format!("[CACHED: DISK] {}", cached)
                }],
                is_error: false
            });
        }
        ...
        if let Ok(Some(cached)) = db.load_plugin_cache(&legacy_key).await {
            state.cache_hits.fetch_add(1, Ordering::Relaxed);
            warn!(
                "⚠️ [MCP-CACHE] Hit LEGACY en Disco para: {}. Migrando...",
                plugin_name
            );
            state
                .plugin_cache
            ...
            return json!(CallToolResult {
                content: vec![McpContent::Text {
                    text: format!("[CACHED: LEGACY_DISK] {}", cached)
                }],
                is_error: false
            });
        }
    ...
    if let Some(p) = plugin {
        ...
        while attempts < max_retries {
            ...
            if attempts < max_retries {
                ...
                warn!(
                    "⚠️ [MCP-RESILIENCIA] Intento {} fallido para {}. Reintentando en {}s...",
                    attempts, plugin_name, delay
                );
        ...
        match scan_result {
            Ok(findings) => {
                ...
                for f in findings.iter() {
                    ...
                    f_filtered.core.description = state
                        .sanitizer
                        .filter_tool_output(plugin_name, &f_filtered.core.description);
                        ...
                        if let Some(obj) = evidence.data.as_object_mut() {
                            if let Some(body) = obj.get_mut("body") {
                                if let Some(s) = body.as_str() {
                                    let filtered_body =
                                        state.sanitizer.filter_tool_output(plugin_name, s);
                ...
                let compressed_findings: Vec<_> = final_findings
                    .iter()
                ...
                let summary = if !support_tone && !support_tonl && !support_wenyan {
                    let optimized = crate::core::ai::PROMPT_OPTIMIZER.optimize(
                        &summary,
                        crate::core::ai::token_optimizer::OptimizationLevel::Full,
                    );
                    let saved_by_opt =
                        crate::core::ai::token_optimizer::PromptOptimizer::savings_tokens(
                            &summary, &optimized,
                        );
                    if saved_by_opt > 0 {
                        state
                            .tokens_saved
                            .fetch_add(saved_by_opt, Ordering::Relaxed);
                        info!(
                            "🔤 [MCP-OPT] PromptOptimizer: ~{} tokens adicionales ahorrados",
                            saved_by_opt
                        );
                    }
                    optimized
                ...
                let content = if support_wenyan {
                    ...
                    McpContent::Text {
                        text: state.sanitizer.mask_output(&summary),
                    }
                };
                ...
                state
                    .plugin_cache
                    .insert(secure_key.clone(), final_text.clone())
                    .await;
                if let Some(ref db) = state.db {
                ...
                state
                    .tokens_saved
                    .fetch_add(token_savings, Ordering::Relaxed);

                info!(
                    "📊 [MCP-STATS] Plugin: {} | Items: {} | Out: {} chars | Saved Tokens: ~{}",
                    plugin_name,
                    findings.len(),
                    final_len,
                    token_savings
                ...
                let safe_content = match &content {
                    McpContent::Text { text } => McpContent::Text {
                        text: to_ascii_safe(text),
                    },
                    McpContent::Wenyan { text } => McpContent::Wenyan {
                        text: to_ascii_safe(text),
                    },
                };
                json!(CallToolResult {
            ...
            Err(e) => {
                state.total_calls.fetch_add(1, Ordering::Relaxed);
                error!(
                    "❌ [MCP-RESILIENCIA] Plugin {} falló definitivamente tras {} intentos: {}",
                    plugin_name, max_retries, e
                );

                if let Some(stale_data) = state
                    .plugin_cache
                    .get(&secure_key)
                    .or_else(|| state.plugin_cache.get(&legacy_key))
                {
                    ...
                    return safe_result(
                        format!(
                            "[MODO_RESILIENCIA: DATOS_HISTORICOS]\nUltimo estado conocido:\n\n{}",
                            stale_data
                        ),
                        false,
                    );
                ...
                safe_result(
                    format!("Error critico de ejecucion (Sin cache disponible): {}", e),
                    true,
                )
            }
        }
