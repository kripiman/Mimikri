...
pub fn validate_path(path_str: &str) -> Result<std::path::PathBuf, String> {
    ...
    let target_path = if path.exists() {
        std::fs::canonicalize(path).map_err(|e| format!("Error de canonicalizacion: {}", e))?
    } else {
        let parent = path
            .parent()
            .ok_or("Ruta sin directorio padre.".to_string())?;
    ...
    if !target_path.starts_with(&workspace_root) {
        return Err(format!(
            "PATH VIOLATION: '{}' fuera del workspace autorizado.",
            path_str
        ));
    }
...
pub async fn handle_route_task(args: serde_json::Value) -> serde_json::Value {
    ...
    let (model, reason, fallback) = if context_tokens > 80_000
        || task.contains("global")
        || task.contains("repo")
        || task.contains("snapshot")
        || files.len() > 15
    {
        (
            "Antigravity (Gemini 1.5 Pro)",
            format!(
                "Massive context ({} tokens) or global reasoning task.",
                context_tokens
            ),
            "Kimi (128k)",
        )
            ...
            }))
    {
        ...
        let reason = if context_tokens > 30_000 {
            format!(
                "Cost-optimized for medium context ({} tokens).",
                context_tokens
            )
        } else {
...
pub async fn handle_smart_read(
    state: &Arc<McpServer>,
    ...
    if state
        .sanitizer
        .filter_tool_output("SmartRead", path_str)
        .is_empty()
    ...
    if let Some(prev_hash) = state.file_hash_cache.get(path_str) {
        if *prev_hash == current_hash {
            ...
            return safe_result(
                format!(
                    "[SMART-READ: NO-CHANGE] '{}' — ~{} tokens ahorrados.",
                    path_str, saved
                ),
                false,
            );
        ...
        state
            .file_hash_cache
            .insert(path_str.to_string(), current_hash);
        let (capped, saved) = cap_content(&content);
        state.tokens_saved.fetch_add(saved, Ordering::Relaxed);
        return safe_result(
            format!("[SMART-READ: CHANGED] '{}'\n{}", path_str, capped),
            false,
        );
    }
...
pub async fn handle_detect_waste(state: &Arc<McpServer>) -> serde_json::Value {
    ...
    let efficiency_ratio = if total_potential > 0 {
        tokens as f32 / total_potential as f32
    } else {
        0.0
    };
    let efficiency_score = efficiency_ratio * 5.0;
    let bloat_penalty = if calls > 10 && efficiency_ratio < 0.1 {
        2.0
    } else {
        0.0
    };
    ...
    safe_result(
        ...
        - Recomendacion: {}",
            ...
            bloat_penalty,
            if score < 5.0 {
                "Ejecutar osint_compress_memory_file inmediatamente."
            } else {
                "Sesion saludable."
            }
        ),
...
pub async fn handle_checkpoint_save(
    state: &Arc<McpServer>,
    args: serde_json::Value,
) -> serde_json::Value {
    ...
    if let Some(prev) = state.checkpoint_manifest.get(trigger) {
        if prev["digest"].as_str() == Some(&digest) {
            return safe_result(
                format!(
                    "[CHECKPOINT: SKIP] Snapshot identico ya existe para '{}'.",
                    trigger
                ),
                false,
            );
    ...
    state
        .checkpoint_manifest
        .insert(trigger.to_string(), entry.clone());

    if let Some(ref db) = state.db {
        let _ = db
            .save_checkpoint(trigger, &entry.to_string(), content)
            .await;
    }

    safe_result(
        format!(
            "[CHECKPOINT: SAVED] Trigger: '{}' | Digest: {}...",
            trigger,
            &digest[..8]
        ),
        false,
...
pub async fn handle_get_stats(state: &Arc<McpServer>) -> serde_json::Value {
    ...
    let loop_warnings: Vec<String> = loop_counts
        .into_iter()
...
pub async fn handle_compress_memory(
    state: &Arc<McpServer>,
    args: serde_json::Value,
) -> serde_json::Value {
    let path_str = args["path"].as_str().unwrap_or("");
    ...
    match tokio::fs::read_to_string(&path).await {
        Ok(content) => {
            if state
                .sanitizer
                .filter_tool_output("MemoryCompressor", &content)
            ...
            match tokio::fs::write(&path, &optimized).await {
                Ok(_) => {
                    state
                        .tokens_saved
                        .fetch_add(saved_tokens, Ordering::Relaxed);
                    safe_result(
                        format!(
                ...
                Err(_) => safe_result(
                    "Error: No se pudo escribir el archivo optimizado.".to_string(),
                    true,
                ),
            }
        }
