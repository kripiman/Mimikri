...
use crate::utils::{liveness::is_safe_ip, LivenessChecker};
...
pub fn spawn_liveness_stage(
    ...
    tokio::spawn(async move {
        ...
        stream
            .for_each_concurrent(concurrency, move |mut target| {
                ...
                async move {
                    ...
                    if let Some(ip) = tokio::select! {
                        ...
                        // CDN Check
                        let cdn_checker =
                            crate::plugins::reconnaissance::active::cdncheck::CdnCheckScanner::new(
                            );
                        if let Ok(true) = cdn_checker.is_cdn(&ip.to_string()).await {
                            info!(
                                "🛡️ CDN GATE: Target {} detected behind CDN/Cloud.",
                                target.host
                            );
