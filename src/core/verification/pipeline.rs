...
use crate::models::{Finding, Severity, TargetHost, ValidationMetadata, ValidationStatus};
...
impl ValidationPipeline {
    ...
    pub async fn check_negative_control(
        ...
        info!(
            "🧬 [Validation] Layer 1: Performing Negative Control with benign payload: '{}'",
            benign_payload
        );
        let client = StealthClientBuilder::build(target, proxy_manager)?;

        let benign_url = url.replace(payload, &benign_payload);
        let res_val: Result<reqwest::Response, reqwest::Error> =
            client.request(method.parse()?, &benign_url).send().await;
        if let Ok(res) = res_val {
            ...
            let expected_pattern = evidence
                .get("expected_pattern")
                .and_then(|v| v.as_str())
                .unwrap_or("");

            if status.as_u16() == original_status
                && !expected_pattern.is_empty()
                && body.contains(expected_pattern)
            {
    ...
    pub async fn verify_proof(
        ...
        if finding.core.title.to_lowercase().contains("lfi")
            || finding.core.title.to_lowercase().contains("inclusion")
        {
            return Ok(Self::verify_lfi(evidence).await);
        }

        if finding.core.title.to_lowercase().contains("rce")
            || finding.core.title.to_lowercase().contains("execution")
        {
    ...
    async fn verify_sqli(
        ...
        if timing > 5000 {
            return Some(format!(
                "Confirmed SQLi via time-delay signature ({}ms).",
                timing
            ));
        }
        ...
        for err in sql_errors {
            if body.to_lowercase().contains(err) {
                return Some(format!(
                    "Confirmed SQLi via error-based signature: '{}'",
                    err
                ));
            }
        }
    ...
    async fn verify_lfi(evidence: &Value) -> Option<String> {
        ...
        {
            return Some(
                "Confirmed LFI via system file marker detection (/etc/passwd or win.ini)."
                    .to_string(),
            );
        }
        None
    ...
    async fn verify_rce(evidence: &Value) -> Option<String> {
        ...
        if body.contains("uid=") && body.contains("gid=")
            || body.contains("Windows IP Configuration")
        {
            return Some(
                "Confirmed RCE via command output verification (id/ipconfig).".to_string(),
            );
        }
        None
    }
    ...
    pub async fn check_oob_verification(
        ...
        if let Some(id) = evidence.get("oob_id").and_then(|v| v.as_str()) {
            info!(
                "🧬 [OOB] checking retrospective hit for existing ID: {}",
                id
            );
            if let Some(hit) = om.wait_for_interaction(id, 5).await? {
                return Ok(Some(format!(
                    "Confirmed OOB interaction ({}) via Retrospective ID: {}",
                    hit.protocol, id
                )));
            }
        }
        ...
        if let (Some(u), Some(p)) = (url, payload) {
            info!(
                "🧬 [OOB] Proactive Re-execution: Triggering fresh PoC with ID: {}...",
                fresh_id
            );
            ...
            let fresh_payload = if p.contains(".interactsh.com") {
                p.split('.')
                    .next()
                    .map(|prefix| p.replace(prefix, &fresh_id))
            ...
            if let Some(hit) = om.wait_for_interaction(&fresh_id, 30).await? {
                return Ok(Some(format!(
                    "Confirmed OOB interaction ({}) via PROACTIVE ID: {}",
                    hit.protocol, fresh_id
                )));
            }
        }
    ...
    pub async fn validate(
        ...
        info!(
            "🔍 [Validation] Invocando Pipeline Anti-Alucinación (V15) para: {}",
            finding.core.id
        );

        let neg_control = Self::check_negative_control(finding, target, &proxy_manager)
            .await
            .unwrap_or_else(|e| {
        ...
        let proof = Self::verify_proof(finding, target, &proxy_manager)
            .await
            .unwrap_or_else(|e| {
        ...
        let oob_proof = Self::check_oob_verification(finding, target, proxy_manager.clone())
            .await
            .unwrap_or_else(|e| {
        ...
        if !neg_control {
            ...
            if verdict.risk_score < 4 {
                ...
                notes = format!(
                    "AI JUDGE VERIFIED: {} | {}",
                    verdict.summary, verdict.stealth_notes
                );
            }
        } else {
            ...
            confidence = 0.5;
            notes =
                "PASSED Negative Control. Skipping Layer 2/3 for low/medium severity.".to_string();
        }
