    ...
    pub async fn sync(&self) -> Result<()> {
        ...
        for (client, handle) in &self.clients {
            ...
            match client.fetch_in_scope(handle).await {
                Ok(scopes) => {
                    tracing::info!(
                        "🛡️ V14.6 SCOPE: Found {} assets for {}",
                        scopes.len(),
                        handle
                    );
                    for s in scopes {
                ...
                Err(e) => tracing::error!(
                    "❌ V14.6 SCOPE: Failed to fetch scope for {}: {}",
                    handle,
                    e
                ),
            }
        ...
        if all_scopes.is_empty() {
            tracing::warn!(
                "🛡️ V14.6 SCOPE: No scopes fetched during sync. Skipping policy update."
            );
            return Ok(());
        }

        let in_scope: Vec<serde_json::Value> = all_scopes
            .into_iter()
            .map(|s| json!({ "target": s }))
            .collect();
        ...
        tracing::info!(
            "🛡️ V14.6 SCOPE: Synchronized {} targets to {:?}",
            in_scope.len(),
            self.policy_path
        );
