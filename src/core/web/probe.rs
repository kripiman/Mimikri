...
pub async fn start_credential_prober(state: Arc<DashboardState>) {
    ...
    loop {
        ...
        if let (Some(user), Some(key)) = (&config.h1_username, &config.h1_api_key) {
            let res = client
                .get("https://api.hackerone.com/v1/me")
            ...
            warn!(
                "⚠️ CREDENTIALS: H1_API_KEY set but H1_USERNAME missing. HackerOne probe skipped."
            );
        }
        ...
        if let Some(key) = &config.bugcrowd_api_key {
            let res = client
                .get("https://api.bugcrowd.com/programs")
        ...
        if let Some(key) = &config.netlas_api_key {
            let res = client
                .get("https://app.netlas.io/api/indices/")
        ...
        if let (Some(key), Some(endpoint)) =
            (&config.antigravity_api_key, &config.antigravity_endpoint)
        {
            let res = client
        ...
        // 14. Azure OpenAI
        if let (Some(key), Some(endpoint)) =
            (&config.azure_openai_key, &config.azure_openai_endpoint)
        {
            let url = format!(
                "{}/openai/models?api-version=2024-10-21",
                endpoint.trim_end_matches('/')
            );
            let res = client.get(&url).header("api-key", key).send().await;
...
async fn update_status(
    ...
    let status = match res {
        ...
        Err(e) => {
            warn!(
                "⚠️ CREDENTIALS: Connection error probing {}: {}",
                service, e
            );
            CredentialStatus {
                service: service.to_string(),
