...
impl<M: ExecutorMode> PocValidator<M> {
    #[allow(dead_code)]
    pub(crate) async fn execute_raw_payload(
        &self,
        payload: &str,
        _target: &TargetHost,
    ) -> Result<String> {
    ...
    pub(crate) async fn execute_safe_command(
        &self,
        payload: &str,
        target: &TargetHost,
    ) -> Result<String> {
        let params: serde_json::Value =
            serde_json::from_str(payload).context("Invalid PoC payload JSON")?;
        ...
        match binary {
            "nmap" => {
                ...
                if let Some(flags) = params["flags"].as_array() {
                    for f in flags {
                        args.push(f.as_str().unwrap().to_string());
                    }
                }
            ...
            "curl" => {
                ...
                if !self.policy.is_path_safe(path) {
                    anyhow::bail!("Policy Block: Unsafe path in curl PoC");
                }
    ...
    pub(crate) async fn execute_tcp_check(
        &self,
        payload: &str,
        target: &TargetHost,
    ) -> Result<String> {
        ...
        let pm = self.proxy_manager.as_ref().context(
            "V13 OPSEC Violation: TCP check requires ProxyManager in Sovereign Stealth mode.",
        )?;
    ...
    pub(crate) async fn execute_icmp_ping(&self, target: &TargetHost) -> Result<String> {
        ...
        let output = self
            ...
            .execute_and_wait(
                "ping",
                vec!["-c".to_string(), "3".to_string(), target_addr.to_string()],
            )
            .await?;
        if output.status.success() {
            Ok("Ping success".into())
        } else {
            Ok("Ping failed".into())
        }
    }
    ...
    pub(crate) async fn execute_http(&self, payload: &str, target: &TargetHost) -> Result<String> {
        let target_ip = target.pinned_addr()?;
        if !self.policy.is_path_safe(payload) {
            anyhow::bail!("Policy Block: Unsafe path in HTTP PoC");
        }

        let pm = self.proxy_manager.as_ref().context(
            "V13 OPSEC Violation: HTTP PoC requires ProxyManager in Sovereign Stealth mode.",
        )?;

        let (_, client) = pm.get_client_fail_closed(&target.host)?;

        let res = client
            .get(format!("http://{}{}", target_ip, payload))
            .send()
            .await?;
        Ok(format!(
            "Status: {}\nBody: {}",
            res.status(),
            res.text().await?.chars().take(1000).collect::<String>()
        ))
    }
}
