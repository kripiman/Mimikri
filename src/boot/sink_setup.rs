...
use mimikri::core::sink::{DataSink, JsonlSink, MultiSink, PostgresSink, TacticalWebhookSink};
...
pub async fn build_multi_sink(
    ...
    if engine_config.h1_api_key.is_some()
        || engine_config.bugcrowd_api_key.is_some()
        || engine_config.intigriti_token.is_some()
    {
    ...
    if let Some(ref nats_url) = args.nats_url {
        if let Ok(nats_sink) =
            mimikri::core::sink::nats_sink::NatsSink::new(nats_url, "mimikri").await
        {
    ...
    if let Some(ref db_url) = args.postgres_url {
        ...
        mimikri::utils::api_budget::ApiBudgetRegistry::init(
            utils_config,
            Some(sink.pool().clone()),
        );
    ...
    if let Ok(c2_env) = std::env::var("C2_URL") {
        if let Ok(parsed_url) = url::Url::parse(&c2_env) {
            if parsed_url.scheme() == "https"
                && is_ssrf_safe_host_async(parsed_url.host_str().unwrap_or("")).await
            {
                let c2_token = std::env::var("C2_TOKEN").ok();
                multi_sink.add(Box::new(TacticalWebhookSink::new(
    ...
    let timeline_path = std::path::PathBuf::from(&utils_config.workspace_dir)
        .join("logs")
        .join("timeline.jsonl");
    if let Ok(activity_log) = mimikri::utils::activity_log::ActivityLog::new(timeline_path).await {
        ...
        multi_sink.add(Box::new(mimikri::core::sink::TimelineSink::new(
            act_log_arc,
        )));
        info!("📝 [TimelineSink] Acitivity log routing attached.");
    } else {
        tracing::warn!(
            "⚠️ [TimelineSink] Failed to initialize ActivityLog at workspace/logs/timeline.jsonl"
        );
    }
