...
pub async fn build_target_stream(
    ...
    let certstream_rx = if !utils_config.certstream_keywords.is_empty() {
        info!(
            "🔱 V14.2 SOVEREIGN: Activating CertStream Daemon for keywords: {:?}",
            utils_config.certstream_keywords
        );
        Some(
            mimikri::infrastructure::certstream::CertStreamDaemon::spawn(
                utils_config.certstream_keywords.clone(),
            ),
        )
    } else {
        None
    ...
    let target_stream: futures::stream::BoxStream<'static, TargetHost> =
        if let Some(apk_path) = args.apk.clone() {
            let cli_scope_id = cli_scope_id_stream.clone();
            let package_name = apk_path
                .split('/')
                .next_back()
                .unwrap_or("mobile_app")
                .to_string();
            ...
            tokio_stream::wrappers::LinesStream::new(tokio::io::AsyncBufReadExt::lines(reader))
                ...
                .filter(|t: &String| {
                    ...
                    if !valid {
                        error!("❌ Skipping invalid target: {}", t_clone);
                    }
                    async move { valid }
                })
                .map(move |t: String| {
            ...
            anyhow::bail!(
            "Either --target, --input, --apk, --dashboard or CERTSTREAM_KEYWORDS must be provided."
        );
        };
        ...
        let cs_stream = tokio_stream::wrappers::ReceiverStream::new(rx)
            ...
            .map(move |t: String| {
                ...
                TargetHost {
                    ...
                    skip_heavy_scan: false,
                    scan_id: None,
                    scope_id: (*cli_scope_id_cs).clone(),
                }
            });
