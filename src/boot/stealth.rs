...
pub async fn init(engine: &RedTeamEngine, args: &Args, utils_config: &Config) -> Result<()> {
    ...
    if let Ok(token) = utils_config.require_do_token() {
        ...
        let shutdown_mgr = Arc::new(mimikri::core::orchestrator::lifecycle::ShutdownManager::new(
        ...
        shutdown_mgr.add_hook(move || {
            Box::pin(async move {
                let do_client = mimikri::infrastructure::digital_ocean::DigitalOceanClient::new(token_clean, pm_clean_hook);
