...
impl ProxyManager {
    pub fn add_managed_exit_with_auth(&self, ip: String, user: &str, pass: &str) {
        ...
        if self.managed_exits.len() >= max_nodes {
            warn!(
                "⚠️ Managed exit pool full ({} nodes). Rejecting {}",
                self.managed_exits.len(),
                ip
            );
            return;
        }
    ...
    pub fn add_managed_exit(&self, ip: String) {
        ...
        info!(
            "🚀 ProxyManager: Active DO Managed Exit added (Anonymous): {}",
            ip
        );
    }
    ...
    pub(crate) fn spawn_local_proxy_client(
        &self,
        remote_ip: &str,
        local_port: u16,
        auth: &str,
        ...
        tokio::spawn(async move {
            ...
            info!(
                "🛡️ STEALTH: Spawning local Hysteria client for {} on port {}...",
...
"#,
                ...
                .spawn()
            {
                Ok(mut child) => {
                    info!(
                        "🚀 STEALTH: Hysteria local client PID {:?} established for {}",
                        child.id(),
                        remote_ip
                    );
