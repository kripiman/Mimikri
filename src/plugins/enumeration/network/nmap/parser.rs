...
use crate::plugins::enumeration::network::nmap::classifier::{
    classify_script_severity, map_category, suggest_exploit_vector,
};
use anyhow::Result;
                ...
                b"service" => {
                    for attr in e.attributes().flatten() {
                        match attr.key.as_ref() {
                            b"name" => {
                                current_service.name = attr
                                    .unescape_value()
                                    .map(|v| v.to_string())
                                    .unwrap_or_default()
                            }
                            b"product" => {
                                current_service.product = attr
                                    .unescape_value()
                                    .map(|v| v.to_string())
                                    .unwrap_or_default()
                            }
                            b"version" => {
                                current_service.version = attr
                                    .unescape_value()
                                    .map(|v| v.to_string())
                                    .unwrap_or_default()
                            }
