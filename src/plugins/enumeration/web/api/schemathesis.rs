...
impl SchemathesisScanner {
    pub fn new() -> Self {
        Self {
            binary_path: detect_tool("schemathesis"),
        }
    }
}
        ...
        match child {
            Ok(c) => {
                let output = c
                    .wait_with_output()
                    .await
                    .context("Failed to wait for schemathesis")?;
                ...
                info!(
                    "SchemathesisScanner: tool finished with exit code {}. Content length: {}",
                    output.status.code().unwrap_or(-1),
                    content.len()
                );

                if !output.status.success() {
                    if content_lower.contains("vulnerability found")
                        || content_lower.contains("falsifying example found")
                    {
                    ...
                    } else if content_lower.contains("too many requests")
                ...
                return Ok(vec![Finding::new(
                    ...
                    Severity::Info,
                    &format!(
                        "Schemathesis terminated without parseable output for {}",
                        target_url
                    ),
                    serde_json::json!({
                        "url": target_url,
