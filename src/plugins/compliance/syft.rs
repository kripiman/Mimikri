...
impl ScannerPlugin for SyftScanner {
    ...
    fn metadata(&self) -> crate::plugins::PluginMetadata {
        crate::plugins::PluginMetadata {
            ...
            description: "SBOM generator using Syft for deep package inventory analysis."
                .to_string(),
    ...
    async fn scan(&self, target: &TargetHost) -> Result<Vec<Finding>> {
        info!(
            "SyftScanner: generating SBOM for container/image: {}",
            target.host
        );
                    ...
                    if let Ok(content) = tokio::fs::read_to_string(&sbom_path).await {
                        if let Ok(json_data) = serde_json::from_str::<serde_json::Value>(&content) {
                            ...
                            findings.push(Finding::new(
                                ...
                                Severity::Info,
                                &format!(
                                    "SBOM generated for {}: {} packages identified",
                                    target.host, total_pkgs
                                ),
                                serde_json::json!({
