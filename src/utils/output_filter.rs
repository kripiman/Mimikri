...
impl CommandFilter {
    ...
    pub fn filter(&self, binary: &str, output: &str, _exit_code: i32) -> String {
        ...
        if output.len() > 5 * 1024 * 1024 {
            return format!(
                "[OUTPUT TOO LARGE (>5MB) - Compression Skipped]\n{}",
                &output[..2000]
            );
        }
    ...
    fn filter_nmap(&self, input: &str) -> String {
        let re_junk =
            Regex::new(r"(?m)^(NSE: |Service scan transition|Initiating |Scanning |Completed ).*$")
                .unwrap();
...
impl SecurityGuard {
    pub fn redact_secrets(input: &str) -> String {
        ...
        static RE_PATTERNS: Lazy<Vec<(Regex, &'static str)>> = Lazy::new(|| {
            vec![
