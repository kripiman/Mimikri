        ...
        Ok(addrs) => {
            for addr in addrs {
                if !is_ip_safe(addr.ip()) {
                    tracing::warn!(
                        "🛡️ SSRF BLOCK: Resolved IP {} for host {} is NOT safe.",
                        addr.ip(),
                        host
                    );
                    return false;
...
pub async fn validate_url_ssrf(url_str: &str) -> anyhow::Result<()> {
    let url = url::Url::parse(url_str).map_err(|_| anyhow::anyhow!("Invalid URL: {}", url_str))?;

    if let Some(host) = url.host_str() {
        if !is_ssrf_safe_host_async(host).await {
            return Err(anyhow::anyhow!(
                "SSRF violation: Host '{}' resolves to a private or loopback IP address.",
                host
            ));
        }
    } else {
